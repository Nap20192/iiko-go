...
func dial(t *testing.T, path, response string, gotBody *string) *organizations.Service {
	t.Helper()
	...
	return organizations.New(c)
}

func TestGetTerminalGroups(t *testing.T) {
	t.Parallel()

	var body string
	s := dial(t, "/api/1/terminal_groups", `{"correlationId":"c-1","terminalGroups":[]}`, &body)

	got, err := s.GetTerminalGroups(context.Background(), gen.TerminalGroupsRequest{OrganizationIDs: []string{"org-1"}})
	if err != nil {
		t.Fatal(err)
	}
